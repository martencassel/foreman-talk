...
sudo hostname set-hostname foreman
echo "172.31.16.1 foreman" | tee -a /etc/hosts > /dev/null
...
# 1. Update the AD realm configuration in /etc/foreman-proxy/settings.d/realm-ad.yml 

# 2. Create the plugin service account and transfer the associated keytab to the Foreman host
